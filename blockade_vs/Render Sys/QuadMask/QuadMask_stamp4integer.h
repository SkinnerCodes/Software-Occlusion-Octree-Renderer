...
template<unsigned int NUM_OF_LINES>
inline void writeMask_stamp4(bareVec4i distanceFromLine[], int64_t& output, int& outputOffset) { //assuming barevec.x is 0-31 in the register
	__m128 result = _mm_castsi128_ps(distanceFromLine[0]);
	for (int i = 1; i < NUM_OF_LINES; i++)
	{
		result = _mm_and_ps(result, _mm_castsi128_ps(distanceFromLine[i]));
	}
	int64_t mask = _mm_movemask_ps(result);
	output |= mask << outputOffset;
}
...
template<unsigned int NUM_OF_LINES>
inline int isQuadFullyOccludedRaster_stamp4(bareVec4i distFromLine[], bareVec4i line2A[], bareVec4i line2B[], int64_t *outputBlock, bareVec4i &minmax) {
	//assumptions made, rasterization starts aligned with block, there min.x() is aligned to a byte, outputOffset = 0 initially, this allows simple block picking, pointer outputBlock +=1
	//all arguments are correct for first stamp
	int columns = minmax.get(2) - minmax.get(0);//in column units to fill, including end and start
	int stampsForRow = (columns >> 1) + (columns & 1); //if it's odd we add 1 so it stamps everything
	...
	bareVec4i firstInRow[NUM_OF_LINES];
	for (int i = 0; i < NUM_OF_LINES; i++) { firstInRow[i] = distFromLine[i]; }
	...
	while (stampsForColumnRemaining > 0)
	{
		...
		firstBlock += (BlocksInRow);
		outputBlock = firstBlock;
		unmodifiedBlock = *outputBlock;
		writeBlock = *outputBlock;
		for (int i = 0; i < NUM_OF_LINES; i++) { distFromLine[i] = firstInRow[i]; }
		for (int i = 0; i < NUM_OF_LINES; i++) { distFromLine[i] = _mm_add_epi32(distFromLine[i], line2B[i]); }
		for (int i = 0; i < NUM_OF_LINES; i++) { firstInRow[i] = distFromLine[i]; }
		outputOffset = 0;
	...
	skipFirst:
		...
		while (stampsForRowRemaining > 16) {
			for (int i = 0; i < 16; i++) {
				...
				outputOffset += 4;
				for (int i = 0; i < NUM_OF_LINES; i++) { distFromLine[i] = _mm_add_epi32(distFromLine[i], line2A[i]); }
			}
...
inline int isQuadFullyOccluded_stamp4(BorderEightLines &border) {
	...
	bareVec4f pixCoordX = _mm_add_ps(_mm_cvtepi32_ps(_mm_shuffle_epi32(minmax, 0b0)), _mm_set_ps(1.5f, 1.5f, 0.5f, 0.5f)); //pixCoordX.x = min.x() +0.5f; pixCoordX.y = min.x()+0.5f; pixCoordX.z = min.x()+1.5f; pixCoordX.w = min.x()+1.5f;
	bareVec4f pixCoordY = _mm_add_ps(_mm_cvtepi32_ps(_mm_shuffle_epi32(minmax, 0b01010101)), _mm_set_ps(1.5f, 0.5f, 1.5f, 0.5f)); //pixCoordY.x = min.y()+0.5f; pixCoordY.y = min.y() + 1.5f; pixCoordY.z = min.y()+0.5f; pixCoordY.w = min.y()+1.5f;
	bareVec4i lineA[NUM_OF_LINES], lineB[NUM_OF_LINES], lineC[NUM_OF_LINES], line2A[NUM_OF_LINES], line2B[NUM_OF_LINES];
	bareVec4i distFromLine[NUM_OF_LINES];
	const int accuracyFactor = 500; const float accuracyFactorF = 500.0f;
	bareVec8i borderAi = _mm256_cvtps_epi32(_mm256_mul_ps(border.A, _mm256_set1_ps(accuracyFactorF)));
	bareVec8i borderBi = _mm256_cvtps_epi32(_mm256_mul_ps(border.B, _mm256_set1_ps(accuracyFactorF)));
	bareVec8i borderCi = _mm256_cvtps_epi32(_mm256_mul_ps(border.C, _mm256_set1_ps(accuracyFactorF * accuracyFactorF)));
	pixCoordX = _mm_mul_ps(pixCoordX, _mm_set1_ps(accuracyFactorF));
	pixCoordY = _mm_mul_ps(pixCoordY, _mm_set1_ps(accuracyFactorF));
	bareVec4i pixCoordXi, pixCoordYi; pixCoordXi = _mm_cvtps_epi32(pixCoordX); pixCoordYi = _mm_cvtps_epi32(pixCoordY);
	__m128i two = _mm_set1_epi32(2 * accuracyFactor);
	for (int i = 0; i < NUM_OF_LINES; i++) {

		lineA[i] = _mm_set1_epi32(borderAi.get(i));
		lineB[i] = _mm_set1_epi32(borderBi.get(i));
		lineC[i] = _mm_set1_epi32(borderCi.get(i));
		distFromLine[i].m128i = getDistances_epi32(lineA[i], lineB[i], lineC[i], pixCoordXi, pixCoordYi); //remember, after the these first determinations are made for the first pixel, the other distances are made just by adding constants

		line2A[i] = _mm_mul_epi32(lineA[i], two);
		line2B[i] = _mm_mul_epi32(lineB[i], two);
	}
	//convert the screen box to a box that is relative to the buffer origin located at the screen regions min corner
	minmax = _mm_sub_epi32(minmax, _mm_set_epi32(ScreenRegionStartY, ScreenRegionStartX, ScreenRegionStartY, ScreenRegionStartX));
	int byteIndex = (minmax.get(0) / 4) + (minmax.get(1) / 2)*(BlocksInRow * 8); // these divisions should be optimized to shifts by compiler
...
inline int isQuadFullyOccluded_stamp4(QuadBorder &border) {
	...
	bareVec4i distFromLine[NUM_OF_LINES];
	const int accuracyFactor = 500; const float accuracyFactorF = 500.0f;
	bareVec4i borderAi = _mm_cvtps_epi32(_mm_mul_ps(border.A, _mm_set1_ps(accuracyFactorF)));
	bareVec4i borderBi = _mm_cvtps_epi32(_mm_mul_ps(border.B, _mm_set1_ps(accuracyFactorF)));
	bareVec4i borderCi = _mm_cvtps_epi32(_mm_mul_ps(border.C, _mm_set1_ps(accuracyFactorF * accuracyFactorF)));
	pixCoordX = _mm_mul_ps(pixCoordX, _mm_set1_ps(accuracyFactorF));
	pixCoordY = _mm_mul_ps(pixCoordY, _mm_set1_ps(accuracyFactorF));
	bareVec4i pixCoordXi, pixCoordYi; pixCoordXi = _mm_cvtps_epi32(pixCoordX); pixCoordYi = _mm_cvtps_epi32(pixCoordY);
	__m128i two = _mm_set1_epi32(2 * accuracyFactor);
